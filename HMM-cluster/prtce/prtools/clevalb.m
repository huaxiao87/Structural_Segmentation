...
% $Id: clevalb.m,v 1.8 2003/11/23 22:31:36 bob Exp $
...
function e = clevalb(a,classf,learnsizes,nreps,fid) 
  ...
  for wi = 1:nw
  	...
  	isuntrained(classf{wi}); name = getname(classf{wi});
  	...
  	for i = 1:nreps
  		...
  		for j = learnsizes
  			...
  			for ci = 1:c
				...
				% Train classifier CLASSF{WI} on this training set and calculate error.
J
  			W = a(J,:)*classf{wi}; 
    		e1(i,li) = testc(a,W);
	...
	prprogress(fid,'\b\bclevalb finished\n')
