...
% $Id: affine.m,v 1.9 2003/10/24 10:08:46 bob Exp $
...
function w = affine(R,offset,lablist_in, lablist_out,size_in,size_out)
	...
	if (nargin == 1) | (~isa(offset,'mapping'))
		...
		if (nargin < 5)
			prwarning(5,'SIZE_IN is not specified. The number of raws of R, %d, is assumed.', m); 
			size_in = m; 
		end
		if (nargin < 4)
		...
		if (size_in == 1) 										% R is a scaling vector
			size_in = size_out; 
		end	
		...
		cost = [];
		if (isa(lablist_in,'dataset'))						% Copy labels from dataset
			cost = lablist_in.cost;
			[lablist_in,size_in] = get(lablist_in,'featlab','featsize');
			size_out = k;
		end
	...
	elseif isa(R,'mapping') 
		...
		w1 = R;
		w2 = offset;
		if (~isclassifier(w1)) & (~isclassifier(w2)) & (strcmp(getmapping_file(w1),'affine'))
