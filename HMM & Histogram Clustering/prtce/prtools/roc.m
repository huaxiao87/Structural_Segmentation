...
%
% Use PLOTR(E) for plotting the result. In the plot the two types of error
...
%	  B = T*NMC(A); E = ROC(T,50); PLOTR(E); % Plots a single curve
%	  E = ROC(T,A*{NMC,UDC,QDC});  PLOTR(E); % Plots 3 curves
%
% REFERENCES
...
%
% SEE ALSO
% DATASETS, MAPPINGS, PLOTR, REJECT, TESTC, GETCLASSI

% Copyright: R.P.W. Duin, duin@ph.tn.tudelft.nl
% Faculty of Applied Sciences, Delft University of Technology
% P.O. Box 5046, 2600 GA Delft, The Netherlands

% $Id: roc.m,v 1.10 2004/12/14 21:04:34 duin Exp $
...
function e = roc(a,w,clas,n)
  ...
  if (~isempty(datname))
	...
	for j = 1:num_w												
		...
		if (isempty(thr))
			thr = [min(d) d(ceil(([1:n-1]/(n-1))*m*c))' 1]; 
		end
	
		e1 = []; e2 = [];

		% NLAB_OUT will be one where B is larger than THR.
		I = matchlablist(b.lablist,b.featlab); % Correct possible changes class orders
		...
		e1 = [e1; mean(errs(find(nlab==clas),:),1)];
		e2 = [e2; mean(errs(find(nlab~=clas),:),1)];
